...
<head>
    ...
    <title>Realm - Domain Management</title>
    <script src="https://cdn.tailwindcss.com"><\/script>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
    <script type="module" src="/src/config/signature.js"><\/script>
    <style>
...
<body class="bg-gray-50 text-gray-800 font-sans antialiased min-h-screen flex flex-col">
    ...
    <!-- Login Screen -->
    <div id="login-screen" class="flex-1 flex items-center justify-center hidden">
        <div class="bg-white p-8 rounded-xl shadow-lg w-full max-w-md border border-gray-100">
            <div class="text-center mb-8">
                <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-blue-100 text-blue-600 mb-4">
                    <i class="fas fa-globe text-2xl"></i>
                </div>
                <h1 class="text-2xl font-bold text-gray-900">Realm</h1>
                <p class="text-gray-500 mt-2">Enter admin password to continue</p>
            </div>
            <form id="login-form" class="space-y-6">
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700">Password</label>
                    <input type="password" id="password" required class="mt-1 block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                </div>
                <div id="login-error" class="text-red-500 text-sm hidden">Invalid password. Please try again.</div>
    ...
    <!-- Dashboard Screen -->
    <div id="dashboard-screen" class="flex-1 hidden flex-col">
        <!-- Header -->
        <header class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-10">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <h1 class="text-xl font-bold text-gray-900">Realm</h1>
                </div>
                <div class="flex items-center gap-4">
                    <button onclick="openModal()" class="inline-flex items-center px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
                        <i class="fas fa-plus mr-2"></i> Add Domain
                    </button>
                    <button onclick="logout()" class="text-gray-500 hover:text-gray-700 transition-colors" title="Logout">
        ...
        <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <div class="text-sm font-medium text-gray-500 mb-1">Total Domains</div>
                    <div class="text-3xl font-bold text-gray-900" id="stat-total">0</div>
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <div class="text-sm font-medium text-gray-500 mb-1">Expiring Soon (< 30 days)</div>
                    <div class="text-3xl font-bold text-red-600" id="stat-expiring">0</div>
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <div class="text-sm font-medium text-gray-500 mb-1">Attention (30-59 days)</div>
                    <div class="text-3xl font-bold text-amber-500" id="stat-attention">0</div>
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <div class="text-sm font-medium text-gray-500 mb-1">Safe (\u2265 60 days)</div>
                    <div class="text-3xl font-bold text-green-600" id="stat-safe">0</div>
                </div>
            </div>
            ...
            <div class="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Domain</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Registrar</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-1/3">Expiration Status</th>
                ...
                <div id="empty-state" class="hidden py-16 text-center">
                    ...
                    <h3 class="text-lg font-medium text-gray-900">No domains found</h3>
                    <p class="mt-1 text-gray-500">Get started by adding your first domain.</p>
                    <button onclick="openModal()" class="mt-6 inline-flex items-center px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 transition-colors">
                        <i class="fas fa-plus mr-2"></i> Add Domain
                    </button>
                </div>
            </div>
    ...
    <!-- Add/Edit Modal -->
    <div id="domain-modal" class="modal fixed inset-0 z-50 invisible opacity-0 flex items-center justify-center bg-gray-900/50 backdrop-blur-sm p-4">
        <div class="modal-content bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-full">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                <h3 class="text-lg font-bold text-gray-900" id="modal-title">Add Domain</h3>
            ...
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-end gap-3">
                <button type="button" onclick="closeModal()" class="px-4 py-2 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
                    Cancel
                </button>
                <button type="button" onclick="saveDomain()" class="px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
                    Save Domain
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        function calculateProgress(purchaseDateStr, expirationDateStr) {
            ...
            return { percentage, daysLeft, colorClass, textClass };
        }
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    \${domain.notes ? \`<div class="text-xs text-gray-500 truncate max-w-xs" title="\${domain.notes}">\${domain.notes}</div>\` : ''}
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap w-1/3">
                            <div class="flex flex-col justify-center">
                                ...
                                <div class="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
                                    <div class="progress-bar-fill h-2.5 rounded-full \${progress.colorClass}" style="width: \${progress.percentage}%"></div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button onclick="editDomain(\${domain.id})" class="text-blue-600 hover:text-blue-900 mr-3 transition-colors" title="Edit">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button onclick="deleteDomain(\${domain.id})" class="text-red-600 hover:text-red-900 transition-colors" title="Delete">
...
(|  '--. |  |\`-' |/  |  '--.(,------.|  |'.'|  |\\_) |  |\\|  | |  |_|( OO ) '..\`''.)(|  '--.
 ...
 \`------'\`------'    \`--'            \`--'   \`--'     \`-----'   \`-----'     \`-----'  \`------'
`;
var figlet_default = FIGLET;

// src/config/signature.js
var _0x1a = [14, 39, 45, 102, 38, 36, 62, 56, 14];
var _0x2b = "aHR0cHM6Ly9iYWxtLmpzLm9yZw==";
var _0x3c = "Realm";
var _0x4d = () => {
  const k = Date.now() % 1e3 === Date.now() % 1e3 ? 75 : 0;
  return _0x1a.map((c) => String.fromCharCode(c ^ k)).join("");
};
var _0x5e = () => {
  try {
    return atob(_0x2b);
...
var _0x6f = () => {
  ...
  if (typeof console !== "undefined" && console.log) {
    console.log(
      "%c" + figlet_default,
      "color: #2563EB; font-family: monospace; font-size: 10px; line-height: 1.2;"
    );
    console.log(
      "%c" + _0x3c,
      "background: linear-gradient(90deg, #1E40AF, #3B82F6); color: #fff; font-size: 16px; padding: 10px 16px; border-radius: 6px; font-weight: bold;"
    );
    console.log(
      "%c\u2728 Created by " + sig + " \u2728 %c\u200B" + site + "\u200B",
  ...
  if (typeof document !== "undefined" && document.head) {
    ...
    authorMeta.setAttribute("content", sig);
    if (site) {
      ...
      authorLink.setAttribute("href", site);
    }
  }
};
if (typeof document !== "undefined") {
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", _0x6f);
  } else {
    setTimeout(_0x6f, 0);
  }
} else {
...
app.get("/api/registrars", (c) => {
  try {
    const safeJson = (c.env.ALLOWED_REGISTRARS || "[]").replace(/'/g, '"');
    const registrars = JSON.parse(safeJson);
    return c.json(registrars);
  } catch (e) {
    return c.json(["Cloudflare", "Namecheap", "\u963F\u91CC\u4E91", "\u817E\u8BAF\u4E91", "GoDaddy"]);
  }
});
app.get("/api/domains", async (c) => {
  const { results } = await c.env.DB.prepare("SELECT * FROM domains ORDER BY expiration_date ASC").all();
  return c.json(results);
});
app.post("/api/domains", async (c) => {
  const { domain_name, registrar, purchase_date, expiration_date, notes } = await c.req.json();
  await c.env.DB.prepare(
    "INSERT INTO domains (domain_name, registrar, purchase_date, expiration_date, notes) VALUES (?, ?, ?, ?, ?)"
  ).bind(domain_name, registrar, purchase_date, expiration_date, notes).run();
  return c.json({ success: true });
});
app.put("/api/domains/:id", async (c) => {
  ...
  const { domain_name, registrar, purchase_date, expiration_date, notes } = await c.req.json();
  await c.env.DB.prepare(
    "UPDATE domains SET domain_name = ?, registrar = ?, purchase_date = ?, expiration_date = ?, notes = ? WHERE id = ?"
  ).bind(domain_name, registrar, purchase_date, expiration_date, notes, id).run();
  return c.json({ success: true });
});
